...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list','diy_list','delete_diy_list','diy_list_goods',///******POWERBY 老杨 QQ:359199843******/
...
/**装机清单**/
elseif ($action == 'diy_list')
{
     include_once(ROOT_PATH . 'includes/lib_clips.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('diy_list').
                                " WHERE user_id='$user_id'");

    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    $smarty->assign('pager', $pager);
    $smarty->assign('diy_list', get_diy_list($user_id, $pager['size'], $pager['start']));
    $smarty->assign('url',        $ecs->url());
    $lang_list = array(
        'UTF8'   => $_LANG['charset']['utf8'],
        'GB2312' => $_LANG['charset']['zh_cn'],
...
/**装机清单详情**/
elseif ($action == 'diy_list_goods')
{
    include_once(ROOT_PATH . 'includes/lib_clips.php');

    $diy_id = isset($_GET['diy_id']) ? intval($_GET['diy_id']) : 0;
	$smarty->assign('diy_id',  $diy_id);
    $smarty->assign('user_id',  $user_id);
    
	$sql = "SELECT user_id, diy_name , diy_id, diy_num , diy_goods ".
            "FROM " . $GLOBALS['ecs']->table('diy_list') .
            " WHERE user_id = '$user_id' AND diy_id = $diy_id";
    $diy = $GLOBALS['db']->getRow($sql);
	
	$good_ids=explode(',',$diy['diy_goods']);
	$good_nums=explode(',',$diy['diy_num']);
...
elseif ($action == 'delete_diy_list')
{
    ...
    if ($diy_id > 0)
    {
        $db->query('DELETE FROM ' .$ecs->table('diy_list'). " WHERE diy_id='$diy_id' AND user_id ='$user_id'" );
    }

    ecs_header("Location: user.php?act=diy_list\n");
    exit;
}
elseif($action=='add_to_cart1') {
