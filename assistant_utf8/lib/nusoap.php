...
class nusoap_base {
	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
		...
		switch(true) {
			...
			case (is_array($val) || $type):
                ...
                if($valueType=='arraySimple' || preg_match('/^ArrayOf/',$type)){ //ereg('^ArrayOf',$type)){ chenfq 20091121
	...
	*/
	function expandQname($qname){
		// get element prefix
		if(strpos($qname,':') && !preg_match('/^http:\/\//',$qname)){ //!ereg('^http://',$qname)){ chenfq 20091120
			// get unqualified name
...
function timestamp_to_iso8601($timestamp,$utc=true){
	...
	if($utc){
		...
		if(preg_match('/'.$eregStr.'/',$datestr,$regs)){//if(ereg($eregStr,$datestr,$regs)){ chenfq 20091120
...
function iso8601_to_timestamp($datestr){
	...
	'([0-9]{2})(\.[0-9]+)?'. // seconds ss.ss...
	'(Z|[+\-][0-9]{2}:?[0-9]{2})?'; // Z to indicate UTC, -/+HH:MM:SS.SS... for local tz's
	//if(ereg($eregStr,$datestr,$regs)){ chenfq 20091120
...
class nusoap_xmlschema extends nusoap_base  {
	...
	function schemaStartElement($parser, $name, $attrs) {
        ...
        if(count($attrs) > 0){
        	foreach($attrs as $k => $v){
				...
				//if(ereg("^xmlns",$k)){ chenfq 20091120
		...
		switch($name){
			...
			case 'complexType':
				...
				if(isset($attrs['name'])){
					...
					//                </sequence>
					//            </complexType>
					//if(isset($attrs['base']) && ereg(':Array$',$attrs['base'])){ chenfq 20091120
    ...
    */
	function getTypeDef($type){
		...
		//} elseif (ereg('_ContainedType$', $type)) { chenfq 20091120
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function soap_transport_http($url, $curl_options = NULL, $use_curl = false){
		...
		//ereg('\$Revisio' . 'n: ([^ ]+)', $this->revision, $rev); chenfq 20091120
	...
	*/
	function setEncoding($enc='gzip, deflate') {
		if (function_exists('gzdeflate')) {
			...
			if(PHP_MAJOR_VERSION < 5 || (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION < 3))// chenfq 20091120 add 
	...
	function getResponse(){
	  ...
	  if ($this->io_method() == 'socket') {
	    ...
	    while (!isset($lb)){
			...
			// remove 100 headers
			//if (isset($lb) && ereg('^HTTP/1.1 100',$data)) { chenfq 20091120
...
//		}
		...
		if ($data == '') {
			...
			$data = $savedata;
			//while (ereg('^HTTP/1.1 100',$data)) { chenfq 20091120
	...
	function parseCookie($cookie_str) {
		...
		$data = explode(';', $cookie_str);//split ==> explode chenfq 20091120
		$value_str = $data[0];
...
class nusoap_server extends nusoap_base {
	 ...
	 * @access public
	 */
	var $decode_utf8 = false; //modfiy by chenfq true -->false 20090427
	/**
	...
	*/
	function service($data){
		...
		if ($rm == 'POST') {
			...
			$this->send_response();
		//} elseif (ereg('wsdl', $qs) ){ chenfq 20091120
	...
	function parse_http_headers() {
		...
		if(function_exists('getallheaders')){
			...
			if(isset($this->headers['content-type']) && strpos($this->headers['content-type'],'=')){
				...
				$enc = str_replace('\\','',$enc);//add by chenfq 2009-05-09
				//if(eregi('^(UTF-8|US-ASCII|UTF-8)$',$enc)){ chenfq 20091120
	...
	*/
    function parseRequest($headers, $data) {
		...
		if (strpos($headers['content-type'], '=')) {
			...
			// should be US-ASCII for HTTP 1.0 or UTF-8 for HTTP 1.1
			//$this->xml_encoding = 'UTF-8';  modify by chenfq 2009-02-25
			$this->xml_encoding = $this->soap_defencoding;			
		}
...
class wsdl extends nusoap_base {
	...
	var $proxypassword = '';
	var $timeout = 0;
	var $response_timeout = 300; // modfiy by chenfq 30-->300 2009-04-27
    ...
    function start_element($parser, $name, $attrs)
    {
        ...
        //} elseif (ereg('schema$', $name)) { chenfq 20091120
	...
	*/
	function end_element($parser, $name){ 
		// unset schema status
		//if (/*ereg('types$', $name) ||*/ ereg('schema$', $name)) { chenfq 20091120
	...
	*/
	...
	{
		$xml = null;
		$xml ='<?xml version="1.0" encoding="'.$this->soap_defencoding.'"?>';//modfiy by chenfq  encoding="UTF-8" -->encoding="'.$this->soap_defencoding.'" 2009-02-25
		$xml .= "\n<definitions";
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		foreach($attrs as $key => $value){
			...
			// if ns declarations, add to class level array of valid namespaces
            if($key_prefix == 'xmlns'){
				//if(ereg('^http://www.w3.org/[0-9]{4}/XMLSchema$',$value)){ chenfq 20091120
			...
			// if it's a type declaration, set type
				...
				*/
				//$expr = '([A-Za-z0-9_]+):([A-Za-z]+[A-Za-z0-9_]+)\[([0-9]+),?([0-9]*)\]'; chenfq 20091120
...
class nusoap_client extends nusoap_base  {
	...
	var $response_timeout = 300; 	// modfiy by chenfq 30-->300 2009-04-27		// HTTP response timeout
    ...
    var $decode_utf8 = false; //modfiy by chenfq true -->false 20090427		// toggles whether the parser decodes element content w/ utf8_decode()
	...
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
		...
		switch(true){
			...
			case preg_match('/^http/',$this->endpoint):				
				...
				//if(ereg('^http:',$this->endpoint)){ chenfq 20091120
				if(preg_match('/^http:/',$this->endpoint)){	
				//if(strpos($this->endpoint,'http:')){
					$this->responseData = $http->send($msg,$timeout,$response_timeout,$this->cookies);
				//} elseif(ereg('^https',$this->endpoint)){ chenfq 20091120
