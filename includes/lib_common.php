...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
	...
	$sql = "select float_percent from necs_category where cat_id=(select cat_id from necs_goods where goods_id={$goods_id})";
    $float_percent = $GLOBALS['db']->getOne($sql);
    if($float_percent){
        //登陆状态
        if(isset($_SESSION['user_id']) && ($_SESSION['user_level']>0)){//已登录

        }else{//未登录
         $goods['shop_price'] =$goods['shop_price'] * (1 + ($float_percent/100));
        }
    }else{
...
function get_brands_cat_diy($cat = '')
{
    $children = "AND ($cat) ";
...
function get_diy_list($user_id, $num = 10, $start = 0)
{

    $sql = "SELECT user_id, diy_name , diy_id ".
            "FROM " . $GLOBALS['ecs']->table('diy_list') .
            " WHERE user_id = '$user_id'";
    $res = $GLOBALS['db']->selectLimit($sql, $num, $start);
    $diy_list = array();
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $diy_list[$row['diy_id']]['diy_id']        = $row['diy_id'];
        $diy_list[$row['diy_id']]['diy_name']  = $row['diy_name'];
    }

    return $diy_list;
}
function get_diy_list_goods($user_id, $diy_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.market_price, g.shop_price AS org_price, '.
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'g.promote_price, g.promote_start_date,g.promote_end_date, c.user_id, c.diy_id, c.diy_name, c.diy_num, c.diy_goods' .
            ' FROM ' . $GLOBALS['ecs']->table('diy_list') . ' AS c' .
            " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ".
...
function get_linked_goods_diy($goods_id)
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date, lg.is_double " .
    ...
    {
		$nowtime=gmtime();
		$real_price=$val['shop_price'];
		if ($val['promote_price'] > 0 && $nowtime > $val['promote_start_date'] && $val['promote_start_date'] <$val['promote_end_date'])
        {
            $real_price = $row[$key]['promote_price'];
        }
		$diy_price=$diy_price + $real_price;
    }

    return $diy_price;
}

function change_linked_goods_diy($goods_id)
{
    $sql = "SELECT link_goods_id " .
	...
	if($row)
	{
		foreach ($row AS $key => $val)
		{
			into_linked_goods_diy($val['link_goods_id']);
...
function into_linked_goods_diy($goods_id)
{
	$sql = "SELECT is_diy FROM " .$GLOBALS['ecs']->table('goods'). " WHERE goods_id='$goods_id' ";
    $is_diy = $GLOBALS['db']->getOne($sql);
	if($is_diy=='1'){
	$shop_price=get_linked_goods_diy($goods_id);
	
	 $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('goods') . " SET shop_price='$shop_price' WHERE goods_id='$goods_id'");

	}
}

/******POWERBY 老杨 QQ:359199843******/
function category_get_goods_diy($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.seller_note, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['selled_count']     = selled_count($row['selled_count']);/******POWERBY 老杨 QQ:359199843******/
		$arr[$row['goods_id']]['com_count']        = com_count($row['goods_id']);/******POWERBY 老杨 QQ:359199843******/
		$arr[$row['goods_id']]['goods_id2']        = get_linked_goods_ly($row['goods_id']);/******POWERBY 老杨 QQ:359199843******/
 ...
 * @return  array
 */
function get_linked_goods_ly($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.cat_id, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
 ...
 function selled_count($goods_id)
{
 ...
 function com_count($goods_id)
{
	 $sql= "select count(*) as count from ".$GLOBALS['ecs']->table('comment')." where id_value='".$goods_id."'";
     $res = $GLOBALS['db']->getOne($sql);
     if($res>0)
     {
     return $res;
...
//获取商品价格
function getShopPrice($goods_id,$attr_id=array(),$num=1){
    $sql = "select shop_price from necs_goods where goods_id={$goods_id}";
	...
	if($attr_id){
	    ...
	    foreach($attr_id as $k=>$v){
	        if($v){
                $sql = "select attr_price from necs_goods_attr where goods_attr_id=".$v;
    ...
    if($float_percent){
        ...
        if(isset($_SESSION['user_id']) && ($_SESSION['user_level']>0)){
            ...
            $price = $shop_price * (1 + ($float_percent/100));
        }
    }else{
		$price = $shop_price;
	}
