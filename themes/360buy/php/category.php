...
function cat_get_goods( $keywords, $children, $brand, $min, $max, $ext, $size, $page, $sort, $order,$float_percent=0 )
{
		...
		while ( $row = $GLOBALS['db']->fetchRow( $res ) )
		{
				...
				// 判断是否登陆 未登录显示浮动价 登陆显示 定价
				if($_SESSION['user_id']>0 && $_SESSION['user_level']>0){
					$arr[$row['goods_id']]['shop_price'] = price_format( $row['shop_price'] );
				}else{
					$percert = 1 + $float_percent/100;
					$arr[$row['goods_id']]['shop_price'] =  price_format(  $row['shop_price'] * $percert );
				}
		}
		return $arr;
}
...
$goodslist = cat_get_goods( $keywords, $children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order,$cat['float_percent'] );
