...
function assign_commentex( $id, $type, $page = 1, $flag = 0 )
{
		...
		while ( ( $row = $GLOBALS['db']->fetchRow( $res ) ) ) // 
		{
				...
				$arr[$row['comment_id']]['re_content'] = $row['re_content'];
...
function get_advlist( $position, $num )
{
		...
		foreach ( $res as $idx => $row )
		{
				...
				$arr[$row['ad_id']]['content'] = "<a href='".$res['ad_link']."' target='_blank'><img src='data/afficheimg/".$row['ad_code']."' width='".$row['ad_width']."' height='".$row['ad_height']."' /></a>";
		}
		return $arr;
}
