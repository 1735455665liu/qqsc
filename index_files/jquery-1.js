(function(a, b) {
	...
	var c = a.document,
		d = function() {
			...
			d.fn = d.prototype = {
				...
				return G.call(b, a)
			}), i.test(" ") && (j = /^[\s ]+/, k = /[\s ]+$/), g = d(c), c.addEventListener ? A = function() {
	...
	function(a, b, c) {
		...
		a && a.type ? (this.originalEvent = a, this.type = a.type, this.isDefaultPrevented = a.defaultPrevented || a.returnValue === !1 || a.defaultPrevented && a.defaultPrevented() ? w : v) : this.type = a, this.timeStamp = d.now(), this[d.expando] = !0
	}, d.Event.prototype = {
...
if (typeof RegExp.prototype.replace != 'function') {
    RegExp.prototype.replace = function (a, b) {
        var c = this.toString();
        var d = c.lastIndexOf('/');
        var e = 0;
        var f = c.split(a).length;
        c = c.replace(a, function (w) {
            if (e == f - 2) {
                e++;
                return b
            }
...
G.util = {
    ...
    regExp: {
        ...
        trim: /(^\s*)|(\s*$)/g,
        zipCode: /^\d{6}$/,
        cellPhone: /^(1[3-8][0-9])\d{8}$/,
        phone: /^0\d{2,3}-\d{7,8}(-\d{1,4})?$/,
        phonereg: /^0?\d{3,4}$/,
        phone1reg: /^\d{7,8}$/,
        phone2reg: /^\d{1,4}$/,
        nickName: /^[0-9A-Za-z_]{4,20}$/,
        time: /^\d{4}-\d{2}-\d{2}$/,
        ReplaceRegExp: function (a, b, c) {
            return a.replace(b, c)
        },
        ValidEmail: function (a) {
            if (a.length > 40 || a.length < 5) {
                return false
            }
            if (G.util.regExp.hanzi.test(a)) {
                return false
            }
            return G.util.regExp.email.test(a)
        },
        ValidPwd: function (a) {
            if (a.length > 16 || a.length < 6) {
                return false
            }
            if (G.util.regExp.hanzi.test(a)) {
                return false
            }
            return !G.util.regExp.tirmAll.test(a)
        }
    },
    classExtend: {
        Clone: function (a) {
        ...
        MixinClass: function (a, b) {
            if (arguments[2]) {
                for (var i = 2, j = b.prototype.length; i < j; i++) {
                    a.prototype[arguments[i]] = b.prototype[arguments[i]]
                }
            } else {
        ...
        (function (g, h) {
            ...
            AjaxRequest.prototype = {
                init: function () {
                    ...
                    if (!h.callback && h.success) {
                        h.callback = h.success
                    }
                    this.url = h.url + g.serialize(h.args || {});
                ...
                validateRequest: function () {
                    for (var a in g.requestQueue) {
                        if (g.requestQueue[a] == this.url) {
                            return false
                        }
                    }
                    g.requestQueue.push(this.url);
                    return true
                },
                beginRequest: function () { 
                    ...
                    $.ajax({
                        ...
                        error: function () { 
                            b.requestDispose();
                            /*刷新页面时jquery ajax abort方法存在bug，此处不宜弹窗提醒错误*/
                            //if (h.error) {
                            //    h.error()
                            //} else {
                               
                            //    console.log(h)
                            //    g.alert(b.requestErrorDefaultMsg, 2)
                            //}
                        }
                    })
                },
                responseValidate: function (a) {
                    if (typeof (a) != 'object') {
                        for (var b in this.IllegalRequestResultData) {
                            var c = this.IllegalRequestResultData[b];
                            var d = c.result;
                            var e = c.msg;
                            var f = c.index;
                            if (f == 1) {
                                if (a.indexOf(d) >= 0) {
                                    g.alert(a, 2, c.callback);
                                    return false
                                }
                            } else {
                                if (a == d) {
                                    g.alert(e, 2, c.callback);
                                    return false
                                }
                            }
                        }
                ...
                requestDispose: function () {
                    for (var i = 0, j = g.requestQueue.length; i < j; i++) {
                        if (g.requestQueue[i] == this.url) {
                            g.requestQueue.splice(i, 1);
                            return
                        }
                    }
                },
    ...
    bindEvent: {
        ...
        mouseleave: 2,
        click: 3,
        MouseHandler: function (c) {
            if (!$(c.delegateId).html()) {
                return false
            }
            $(c.delegateId).find(c.mouseenterId).css({
                cursor: 'pointer'
            });
            var d = $(c.delegateId).delegate(c.mouseenterId, c.eventType, function () {
                Model.delegateId = $(this).index();
                var b = this;
                if (!$(this).hasClass(c.toggleClass)) {
                    setTimeout(function () {
                        if (Model.delegateId == $(b).index()) {
                            if (c.toggleClass) {
                                if (c.find) {
                                    $(b).find(c.find).addClass(c.toggleClass).siblings().find(c.find).removeClass(c.toggleClass)
                                } else {
                                    $(b).addClass(c.toggleClass).siblings().removeClass(c.toggleClass)
                                }
                            }
                            if ($(c.handlerId + '> div') && $(c.handlerId + '> div').size() > 0) {
                                var a = $(b).attr("data-id");
                                if (c.handlerClass) {
                                    $(c.handlerId + '> div').addClass(c.handlerClass);
                                    $(c.handlerId + '> div[id^=' + a + ']').removeClass(c.handlerClass)
                                } else {
                                    $(c.handlerId + '> div').hide();
                                    $(c.handlerId + '> div[id^=' + a + ']').show()
                                }
                            } else {
                                $(c.handlerId).show()
                            }
                            if (c.callback) {
                                c.callback.call(c.that, G.util.bindEvent[c.eventType], b)
                            }
                        }
                    }, typeof c.waitTime == 'number' ? c.waitTime : 400)
                }
            });
            if (c.eventType == 'mouseenter') {
                d.delegate(c.mouseenterId, 'mouseleave', function () {
                    var a = this;
                    Model.delegateId = null;
                    if (c.leave && c.callback) {
                        if ($(c.handlerId + '> div') && $(c.handlerId + '> div').size() > 0) {
                            $(c.handlerId + '> div').hide()
                        } else {
                            $(c.handlerId).hide();
                            if (c.find) {
                                $(a).find(c.find).removeClass(c.toggleClass)
                            } else {
                                $(a).removeClass(c.toggleClass)
                            }
                        }
                        c.callback.call(c.that, G.util.bindEvent.mouseleave, a)
                    }
                })
            }
        }
    },
    lazyLoad: (function (g, w) {
        var e = {
            attrArray: ['_src'],
            threshold: 500
        };
        var p = function (v) {
            for (var k in e.attrArray) {
                if (e.attrArray[k] == v) {
                    return
                }
            }
        ...
        g(function ($) {
            $(w).bind('scroll', l).triggerHandler('scroll')
        });
        return {
            run: function (v) {
    ...
    replace: function (c, d) {
        return d.replace(/{([^{}]*)}/g, function (a, b) {
            if (c[b]) {
                return c[b]
            }
            return a
    ...
    alert: function (v, a, e) {
        ...
        var m = $('#dgmsg'),
			...
			dbg = $('#dialogbaseMsg'),
			h, c = '',
			w = d.css('width').replace('px', ''),
			dh = d.css('height').replace('px', ''),
			dt = $(document).height(),
			cw = document.body.clientWidth || document.documentElement.clientWidth,
			dl = document.body.scrollLeft || document.documentElement.scrollLeft,
			ch = document.body.clientHeight || document.documentElement.clientHeight;
        if (a == 1) {
            c = 'icon-msg icon-msg-right'
        } else if (a == 0) {
            c = 'icon-msg icon-msg-warn'
        } else if (a == 2) {
            c = 'icon-msg icon-msg-wrong'
        } else if (a == 3) {
            c = 'none'
        } else {
            c = 'icon-msg icon-msg-warn'
        ...
        tg.html('提示');
        m.attr('class', c);
        if (typeof v == 'string' || v.AddproductInfoList || v.ErrorproductInfoList) {
            var f = {
                msg: typeof v == 'string' ? v : ''
            };
            var p = false;
            if (v.ErrorproductInfoList) {
                var k = '';
                for (var i in v.ErrorproductInfoList) {
                    with (v.ErrorproductInfoList[i]) {
                        k += '商品：' + BriefName + '<br />'
                    }
                }
                f.msg += k
            }
            if (v.AddproductInfoList) {
                var k = '';
                for (var i in v.AddproductInfoList) {
                    p = true;
                    with (v.AddproductInfoList[i]) {
                        k += '商品：' + BriefName + '<br />'
                    }
                }
                f.msg += k
            ...
            mc.html(f.msg).show();
            if (p || v.ErrorproductInfoList) {
                if (v.ErrorproductInfoList && !p) {
                    mc.parent().append('<p dgmsg-body-notice>加入购物车失败</p>')
                } else {
                    mc.parent().append('<p dgmsg-body-notice>已经成功加入购物车</p>')
                }
                //mc.parent().append('<p class="pt20">购物车当前共有<span class="red">' + v.count + '</span>件商品，合计金额（不含折扣）：<span class="price">' + v.price + '</span></p>')
            }
            cm.show();
            cm_btn.show();
            cm_btn.html("确定");
        ...
        dh = d.css('height').replace('px', '') / 2;
        dh = '-' + dh + 'px';
        d.css('margin-top', dh);
        d.show();
        if (G.util.ie6alert()) {
            gv.html('<iframe style="position:absolute;width:100%;height:100%;_filter:alpha(opacity=0.5);opacity=0.5;border-style:none;background-color:black"></iframe>')
        }
        gv.css({
            'opacity': 0.5,
            "height": dt,
    ...
    close: function (k) {
        $('#dialog').html(Model.dialog);
        $('.dgmsg-main').html('<h6 id="confirmmsg"></h6>');
        $('#dgmsg').removeAttr('class');
        if (!k) {
            $('#dialog').hide();
            $('#greybackground').hide()
        }
    },
    reviewFrom: function (a) {
        var d = $('#reviewFrom'),
			g = $("#greybackground"),
			dt = $(document).height(),
			w = d.css('width').replace('px', ''),
        ...
        g.css({
            ...
            "width": cw + dl,
            'display': 'block'
        }).html(G.util.ie6alert());
        if (a) a()
    },
    reviewFromClose: function () {
        $('#reviewFrom').hide();
        $('#greybackground').hide()
    },
    cookie: {
        ...
        set: function (k, v, t) {
            var a = new Date();
            a.setTime(a.getTime() + this.expires * 60 * 60 * 1000);
            //这里仅针对cookie过期时间为session结束时间，不做其他cookie过期时间扩展，但保留扩展可能性 chenqifeng
            if (!!t) {
        ...
        remove: function (k) {
            ...
            document.cookie = k + '=;expires=' + a.toGMTString()
        }
    },
    imgMove: (function () {
        var l, r, o, s, u, w, z, x, c, back;
        var k = function () {
            u.delegate(c, 'mouseover', function () {
                u.find(c).removeClass(o);
                if (back) {
                    back($(this))
                }
                $(this).addClass(o)
            ...
            w = parseInt(u.find(c).css('width').replace('px', ''));
            var a = u.find(c).css('margin-left').replace('px', '');
            var b = u.find(c).css('margin-right').replace('px', '');
            if (Number(a)) {
                w += parseInt(a)
            }
            if (Number(b)) {
            ...
            x = z * w - u.find(c).size() * w
        };
        var m = function () {
            var i = parseInt(u.css('left').replace('px', ''));
            i += w;
            if (i >= 0) {
                i = 0;
        ...
        var n = function () {
            var j = i = parseInt(u.css('left').replace('px', ''));
            if (!i) {
                i = 0
            }
            i -= w;
        ...
        return {
            run: function (a, b, d, e, f, g, h, i) {
                l = $(b), r = $(d), o = e, s = h, u = $(a), z = f, c = g, back = i;
                k()
            }
        }
    })(),
    Area: (function () {
        var h = function (b, c, d, e, f) {
            this.f = e, this.t = c, this.s = d, this.d = b, this.l = f;
            if (!e) this.f = '';
            var g = this;
            G.util.ajax({
                url: '/common/area',
        ...
        h.prototype = {
            Province: function (a) {
                if (a) {
                    $('#' + this.d + 'province').val(a)
                }
                return $('#' + this.d + 'province').val()
            },
            City: function (a) {
                if (a) {
                    $('#' + this.d + 'city').val(a)
                }
                return $('#' + this.d + 'city').val()
            },
            District: function (a) {
                if (a) {
                    $('#' + this.d + 'district').val(a)
                }
                return $('#' + this.d + 'district').val()
            },
            ProvinceText: function () {
                return $('#' + this.d + 'province').find("option:selected").text()
            },
            CityText: function () {
                return $('#' + this.d + 'city').find("option:selected").text()
            },
            DistrictText: function () {
                return $('#' + this.d + 'district').find("option:selected").text()
            }
        };
        return {
            loadArea: function (a, b, c, d, e) {
...
(function ($, l) {
    ...
    $(function () {
        ...
        }).delegate('[data-type=num]', 'keyup', function () {
            var a = $(this).val();
            var v = a.replace(new RegExp(/[^\d]/g), '');
            if (v != a) {
                $(this).val(v)
            }
    ...
    n.create = function (i) {
        ...
        j.fn.inits = function () { };
        j.fn.proxy = j.proxy;
        j.include = function (a) {
            $.extend(this.fn, a)
        };
        j.extend = function (a) {
            $.extend(this, a)
        };
        j.updateView = function (a, b) {
            if (a) {
                a();
                var c = this;
                setTimeout(function () {
        ...
        j.include({
            ...
            refreshElements: function () {
                for (var b in this.elements) {
                    ...
                    if (typeof (this.elements[b]) != 'object') {
                        this[this.elements[b]] = this.$(b);
                        this[p + this.elements[b]] = b
                    } else {
                        for (var a in this.elements[b]) {
                            this[this.elements[b][a]] = this.$(a);
                            this[p + this.elements[b][a]] = a
                        }
                    }
                }
            },
            eventSplitter: /^([^\s]*)\s*([^\s]*)\s*([^\s]*)\s*(.*)$/,
            delegateEvents: function () {
                for (var h in this.events) {
                    (function (b) {
                        var c = b.events[h];
                        var d = b.proxy(b[c]);
                        var f = h.match(b.eventSplitter);
                        var g = '',
							eventName = '';
                        if (f[4]) {
                            g = f[1] + ' ' + f[2] + ' ' + f[3];
                        ...
                        b.el.delegate(g, eventName, function (e) {
                            (function (k, a) {
                                d.call(b, k, (a || window.event))
                            })(this, e)
                        })
                    })(this)
                }
            ...
            bindAppEvents: function () {
                for (var a in this.appEvents) {
                    var b = this.appEvents[a];
                    var c = this.proxy(this[b]);
                    App.bind(a, c)
                }
            },
            bindMouseEvents: function () {
                for (var d in this.mouseHandler) {
                    var b = this.mouseHandler[d];
                    (function (t, s) {
                        var a = s.callback;
                        if (a) {
                            a = t[a]
                        ...
                        s.callback = a;
                        s.that = t;
                        G.util.bindEvent.MouseHandler(s)
                    })(this, b)
                }
            }
        });
        ...
        if (a.length == 1) {
            ...
            if (c.length > 1) {
                for (var i = 0, j = c.length; i < j; ++i) {
                    b.push(decodeURIComponent(c[i]))
                }
            } else {
                b.push(decodeURIComponent(p))
            }
        } else {
...
function showKF() {
	window.open("http://tb.53kf.com/webCompany.php?arg=10003907&style=1&kflist=off&kf=&zdkf_type=1&language=cn&charset=gbk&username=&userinfo=&introurl=&lyurl=&lytype=0&copartner=&referer=file%3A%2F%2F%2FC%3A%2FUsers%2Fliuxp%2FDesktop%2Fs.html&keyword=&brief=&logo=&question=&tfrom=2&timeStamp=1365483401988");
}