...
if (empty($_GET['encode']))
{
    ...
    header("Location: newsales.php?encode=$string\n");
...
else
{
    ...
    $default_sort_order_method = 'DESC';//$_CFG['sort_order_method'] == '0' ? 'DESC' : 'ASC';
    $default_sort_order_type   = 'add_time';//$_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');
    ...
    if($float_percent){
        ...
        if(isset($_SESSION['user_id']) && ($_SESSION['user_level']>0)){//已登录
           ...
           $arr[$row['goods_id']]['market_price'] = $row['market_price'] * (1 + ($float_percent/100));
        }
    }else{
		$arr[$row['goods_id']]['market_price']  = price_format($row['market_price']);
	}
        ...
        $arr[$row['goods_id']]['shop_price']    =1;//price_format($row['shop_price']);
