...
function order_fee($order, $goods, $consignee)
{
		 ...
		 foreach ($goods AS $val)
    {
		...
		$total['exchange_integral'] += $exchange_integral* $val['goods_number'];
...
function update_order2($order_id,$money_paid)
{
    $_time_ = time();
    $order = array(
        "order_status" => 5,
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_action'), $status_info, 'INSERT');
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $k=>$v){
        //$arr[$k]['goods_price'] = getShopPrice($v['goods_id']);
        $arr[$k]['subtotal'] = $arr[$k]['goods_price'] * $v['goods_number'];
    }
