...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if(!empty($ec_salt))
    {
         ...
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt".
            ...
            " WHERE user_name = '" . $_POST['username']. "' or password = '" . md5(md5($_POST['password']).$ec_salt) . "'";
    }
    else
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt".
            " FROM " . $ecs->table('admin_user') .
            " WHERE user_name = '" . $_POST['username']. "' or password = '" . md5($_POST['password']) . "'";
    }
    $row = $db->getRow($sql);
    if ($row)
    {
