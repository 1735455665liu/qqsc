...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    $json   = new JSON;
    $res    = array('err_msg' => '', 'result' => '', 'qty' => 1, 'proprice' => 0);
    ...
    else
    {
		...
		$shop_price  = getShopPrice($goods_id,$attr_id);
        $res['result'] = price_format($shop_price * $number);
		
		$sql = "SELECT promote_price,promote_start_date,promote_end_date  FROM " . $GLOBALS['ecs']->table('goods') .
        " WHERE goods_id='$goods_id'";

		$promote_info= $GLOBALS['db']->getRow($sql);
		$promote_start_date=$promote_info['promote_start_date'];
		$promote_end_date  =$promote_info['promote_end_date'];
		$nowtime=time();
		if($nowtime>$promote_start_date&&$nowtime<$promote_end_date){
			$promote_price=$promote_info['promote_price'];
			$res['proprice'] = price_format($promote_price * $number);
		}
