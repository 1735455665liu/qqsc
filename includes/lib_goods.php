...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['market_price'] = price_format(ceil($row['market_price']));
            
			
			
			$sql = "select float_percent from necs_category where cat_id=(select cat_id from necs_goods where goods_id={$row[goods_id]})";
    ...
    if($float_percent){
        ...
        if(isset($_SESSION['user_id']) && ($_SESSION['user_level']>0)){//已登录
         ...
         $goods[$idx]['shop_price'] = $row['shop_price'] * (1 + ($float_percent/100));
        }
    }else{
		$goods[$idx]['shop_price']  = price_format($row['shop_price']);
	}
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		$row['sold_num'] = get_sold_num($goods_id);
        return $row;
    }
    else
    {
 ...
 *
 */
function get_sold_num0($goods_id){
	$sql = 'select count(goods_id) as sold_num from'.$GLOBALS['ecs']->table('order_goods').'where goods_id='.$goods_id;
	$row = $GLOBALS['db']->getRow($sql);
	$sql2 = 'select sale_num from'.$GLOBALS['ecs']->table('goods').'where goods_id='.$goods_id;
	$row2 =  $GLOBALS['db']->getRow($sql2);
	return ($row['sold_num'] + $row2['sale_num']) * 9;
}
function get_sold_num($goods_id){
	$sql = 'select sum(goods_number) as sold_num from'.$GLOBALS['ecs']->table('order_goods').'where goods_id='.$goods_id;
	$row = $GLOBALS['db']->getRow($sql);
	$sql2 = 'select sale_num from'.$GLOBALS['ecs']->table('goods').'where goods_id='.$goods_id;
	$row2 =  $GLOBALS['db']->getRow($sql2);
	return ($row['sold_num'] + $row2['sale_num']) * 9;
